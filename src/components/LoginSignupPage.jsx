...
const LoginSignupPage = () => {
  ...
  const validateForm = () => {
    ...
    if (isLogin) {
      ...
      if (!formData.email) { setError('Please enter your email.'); return false; }
      ...
      const hasUpperCase = /[A-Z]/.test(formData.password); const hasLowerCase = /[a-z]/.test(formData.password);
      const hasNumbers = /\d/.test(formData.password); const hasSpecialChar = /[!@#$%^&*(),.?":{}|<>]/.test(formData.password);
      if (!hasUpperCase || !hasLowerCase || !hasNumbers || !hasSpecialChar) { setError('Password needs uppercase, lowercase, number, and special character.'); return false; }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      toast.success('Login Successful!', { position: "bottom-right", autoClose: 2000, hideProgressBar: false, /* etc */ });
      localStorage.setItem('userEmail', formData.email);
      // Optional: Store name if available
      if (formData.name) { // Simple check if name field was somehow populated during login attempt (unlikely but safe)
        ...
        <>
            ...
            {particles.map(particle => (
              <div
                ...
                style={{
                  ...
                  opacity: particle.opacity, filter: 'blur(1px)', backgroundColor: '#8685ef',
                }}
              />
            ))}
          </div>
          ...
          <div className="relative z-10 w-full max-w-md p-6 sm:p-8 bg-black/80 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-800">
            ...
            <div className="mb-6 sm:mb-8 text-center">
              <h1 className="text-2xl sm:text-3xl font-bold mb-1" style={{ color: '#8685ef', fontFamily: 'IBM Plex Mono, monospace' }}>
                {isLogin ? 'Welcome Back' : 'Create Account'}
              </h1>
              <p className="mt-1 text-sm sm:text-base text-white" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>
                {isLogin ? 'Sign in to continue' : 'Get started by creating your account'}
              </p>
            </div>

            {/* Error/Success Messages */}
            {error && <div className="mb-4 p-3 bg-red-900/30 border border-red-500/50 rounded-lg text-red-300 text-sm font-medium text-center" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>{error}</div>}
            {success && <div className="mb-4 p-3 bg-green-900/30 border border-green-500/50 rounded-lg text-green-300 text-sm font-medium text-center" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>{success}</div>}
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-4">
              ...
              {!isLogin && (
                <div>
                  <label className="block mb-1.5 text-xs sm:text-sm text-gray-300" htmlFor="name" style={{ fontFamily: 'IBM Plex Mono, monospace' }}> Your Name </label>
                  <input id="name" name="name" type="text" required
                    ...
                    style={{ fontFamily: 'IBM Plex Mono, monospace' }} autoComplete="off"
                  />
                </div>
              )}
              ...
              {/* Email Input */}
              <div>
                <label className="block mb-1.5 text-xs sm:text-sm text-gray-300" htmlFor="email" style={{ fontFamily: 'IBM Plex Mono, monospace' }}> Email Address </label>
              ...
              {/* Password Input */}
              <div>
                <label className="block mb-1.5 text-xs sm:text-sm text-gray-300" htmlFor="password" style={{ fontFamily: 'IBM Plex Mono, monospace' }}> Password </label>
                <div className="relative">
                  <input id="password" name="password" required
                    ...
                    style={{ fontFamily: 'IBM Plex Mono, monospace' }} autoComplete="off"
                  />
                  <button type="button" // IMPORTANT: type="button"
              ...
              {!isLogin && (
                <div>
                  <label className="block mb-1.5 text-xs sm:text-sm text-gray-300" htmlFor="confirmPassword" style={{ fontFamily: 'IBM Plex Mono, monospace' }}> Confirm Password </label>
                  <div className="relative">
              ...
              {isLogin && (
                <div className="text-right pt-1"> {/* Added slight padding top */}
                  <a href="#" className="text-xs sm:text-sm text-purple-400 hover:text-purple-300 transition-colors"
                    onClick={(e) => { e.preventDefault(); alert("Forgot Password clicked - not implemented."); }}
              ...
              <button type="submit" disabled={loading}
                ...
                style={{ boxShadow: '0 4px 14px rgba(134, 133, 239, 0.25)', fontFamily: 'IBM Plex Mono, monospace' }}
              >
            ...
            <div className="mt-6 text-center">
              <p className="text-xs sm:text-sm text-gray-400" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>
                {isLogin ? "Don't have an account?" : "Already have an account?"}
                <button onClick={toggleMode} className="ml-1.5 font-medium transition-colors hover:text-purple-300 focus:outline-none focus:underline"
              ...
              <div className="mt-6 pt-6 border-t border-gray-700/50">
                <div className="relative mb-4">
                   ...
                   <div className="relative flex justify-center">
                       <span className="bg-black/80 px-2 text-xs sm:text-sm text-gray-400" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>Or continue with</span>
                   </div>
                </div>
                <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
                  <button /* Google */
                    ...
                    onClick={(e) => { e.preventDefault(); alert("Google Sign-in clicked - not implemented."); }}
                    style={{ fontFamily: 'IBM Plex Mono, monospace' }} >
                    <GoogleIcon /> <span className="ml-1.5 sm:ml-2">Google</span>
                  </button>
                  <button /* Twitter */
                    className="flex items-center justify-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-gray-800/60 border border-gray-700 hover:border-purple-500/70 transition-colors text-white text-xs sm:text-sm focus:outline-none focus:ring-1 focus:ring-purple-500/50"
                    onClick={(e) => { e.preventDefault(); alert("Twitter Sign-in clicked - not implemented."); }}
                    style={{ fontFamily: 'IBM Plex Mono, monospace' }} >
                    <TwitterIcon /> <span className="ml-1.5 sm:ml-2">Twitter</span>
                  </button>
                </div>
              </div>
            )}
